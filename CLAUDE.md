...
- `.github/workflows/ci-lima.yml` — the real VM via `make vm` / `make provision` / `make test`, ~30 min. Uses `lima-vm/lima-actions/setup@v1`, which installs qemu and chowns `/dev/kvm` (nested virt works on public-repo runners). This is the only place the tools actually _run_: podman pulls and runs a container, skopeo syncs an image, and pytest drives a full crun lifecycle. It is also the only place that exercises the **unprivileged-user** path.

- `.github/workflows/lint.yml` — ansible-lint plus a prettier format check, about a minute. The playbook is expected to pass at ansible-lint's **production** profile; the only suppressed rule is `latest[git]`, which is inline-noqa'd because tracking each fork's default branch is the design.
...
**Build caching lives only in the Docker job**, and only saves on `main`. Restores happen on every branch and fall back to main's entry. A cache entry is gigabytes against a 10 GB repo-wide LRU budget, so letting every branch save would evict both main's entry and the lima job's guest image. The lima job is deliberately uncached: its build runs inside the VM, so the caches would have to cross a reverse-sshfs mount, where thousands of small files are likely slower than rebuilding. Note the apt consolidation in `34502fd` did **not** measurably speed the build (18m46s vs an 18m37s baseline) — this build is compilation-bound, so only the compiler caches matter.

**`container-lab.yaml`** inherits digest-pinned Debian images from lima's own `template:_images/debian-13`, so image updates come from upgrading lima. It deliberately mounts nothing by default; `make vm` adds a single read-only mount of the checkout via `limactl create --mount "$(CURDIR)"`, which lands at the _same path_ in the guest — safe only because lima gives the guest a `/home/<user>.guest` home that can never collide with the guest's own `~/code`. The `make` targets rely on that identical path via `limactl shell --workdir`. Template provisioning installs prerequisites only; the playbook runs from `make provision` so VM creation stays fast and the long build is retryable without recreating the VM. Its readiness probe waits for cloud-init, or `make provision` races apt for the dpkg lock.
